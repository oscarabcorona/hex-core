...
export const useStreamingMessageSchema: ComponentSchemaDefinition = {
	name: "use-streaming-message",
	displayName: "useStreamingMessage",
	description:
	...
	props: [
		{
			...
			required: true,
			description:
				"UseAIChatReturn — the value from useAIChat() (or any adapter returning the same shape).",
		},
		{
			name: "messageId",
			type: "string",
			required: true,
			description:
				"The id of the message to introspect. Returns isStreaming: false when no message matches.",
		},
	],
	variants: [],
	slots: [],
	...
	examples: [
		{
			title: "Loading indicator per bubble",
			description:
				"Show a streaming indicator inside the message that's currently being generated.",
			code: `import { useAIChat, useStreamingMessage, Message, LoadingIndicator } from "@hex-core/components";
  ...
  return (
    <Message role={message.role}>
      ...
      {isStreaming ? <LoadingIndicator variant="dots" /> : null}
			...
			composition: ["chat", "hook", "streaming", "loading-indicator"],
		},
	],
	ai: {
		whenToUse:
