/**
 * `emit_figma_tokens` tool — render a Hex Core theme as a Figma Variables REST
 ...
 *   - one variable collection ("Hex Core — <theme>")
...
export function buildFigmaPayload(theme: FigmaTokensTheme): FigmaPayloadResult {
	...
	// Collection name uses an en-dash for human readability ("Hex Core — Default").
	...
	const variableCollections: FigmaVariablesPayload["variableCollections"] = [
		{
			action: "CREATE",
			id: collectionId,
			name: `Hex Core — ${theme.displayName}`,
...
export function buildFigmaTokens(input: FigmaTokensInput): string {
	...
	lines.push("# Figma tokens — Hex Core");
	lines.push("");
	lines.push(
