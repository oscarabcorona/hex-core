...
Two known gaps this does **not** close: prop-shape drift (TS2322, e.g. the `Pagination` API above) needs a real typecheck over the generated tree; and ~40 recipe-referenced items whose examples are bare JSX with no imports would still emit an unimported component if composed as a section.
