...
- Composing (4): `Bounce`, `Shimmer`, `Stagger`, `RevealOnScroll`
...
- `tsup` config now uses fast-glob to emit per-component subpath bundles, so `import { FadeIn } from "@hex-core/motion/fade-in"` tree-shakes cleanly
...
- Visual regression test now forces `prefers-reduced-motion: reduce` so motion-heavy demos render deterministically at their final state.
- 9th `hex-core-motion` SKILL.md gains a Catalog table for AI agent decision making.
